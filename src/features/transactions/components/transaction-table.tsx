...
export function TransactionTable({
  ...
  return (
    <Card>
      ...
      <CardContent>
        {items.length === 0 ? (
          <p className="text-sm text-muted-foreground">Belum ada transaksi yang bisa ditampilkan.</p>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableRow key={transaction.id}>
                  <TableCell>
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{transaction.note || "Tanpa catatan"}</p>
                    </div>
                  </TableCell>
                  <TableCell>{transaction.walletName}</TableCell>
                  <TableCell>{formatShortDate(transaction.transactionDate)}</TableCell>
                  <TableCell>
                    <Badge variant={typeVariant(transaction.type)}>{transaction.type}</Badge>
                  </TableCell>
                  <TableCell
                    className={`text-right font-medium ${
                      transaction.type === "expense" ? "text-amber-600 dark:text-amber-400" : "text-emerald-600 dark:text-emerald-400"
                    }`}
                  >
